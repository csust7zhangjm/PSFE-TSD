...
class Dense_Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        # x3 = self.cv3(x2) + x2
        x4 = self.cv4(x1) + x1 + x
        return x4 if self.add else self.cv4(self.cv1(x))
...
class Bottleneck_IFResNet(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv3 = Conv(4 * c_, c2, 3, 1, g=g)
        self.cv4 = Conv(c1, c2, 1, 1)


    def forward(self, x):
        x1 = x2 = x3 = x4 = self.cv1(x)
        k1 = self.cv2(x1)
        x2 = k1 + x2
        k2 = self.cv2(x2)
        x3 = k2 + x3
        ...
        y1 = k1 + y2
        y1 = self.cv2(y3)
        a = torch.cat((y1,y2,y3,y4), 1)
        return self.cv4(x) + self.cv3(a)


class C3_IFResNet(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck_IFResNet(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
